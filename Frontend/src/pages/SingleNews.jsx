...
const SingleNews = () => {
  ...
  const fetchTrendingNews = async () => {
    try {
      const response = await axios.get(`${import.meta.env.VITE_API_URL}/news/trending`, {
        params: { limit: 5 }
      })
      setTrendingNews(response.data.news || [])
    } catch (error) {
      console.error('Error fetching trending news:', error)
      // Fallback to fetching regular news if trending endpoint doesn't exist
      try {
        const fallbackResponse = await axios.get(`${import.meta.env.VITE_API_URL}/news`, {
          params: { limit: 5, sort: 'views' }
        })
        setTrendingNews(fallbackResponse.data.news || [])
      } catch (fallbackError) {
    ...
    <>
      ...
      >
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <motion.article className="flex-1" variants={fadeInUp}>
            {/* Header */}
            <motion.header 
              className="mb-8"
              variants={fadeInUp}
          ...
          <motion.aside 
            className="lg:w-80 xl:w-96 hidden lg:block space-y-6"
            variants={fadeInUp}
          >
            <div className="sticky top-24 space-y-6">
              {/* Related News Section */}
              <div className="bg-white dark:bg-gray-800 rounded-lg p-6 shadow-lg">
                <h2 className="text-xl font-bold mb-4 text-gray-900 dark:text-white border-b pb-2 flex items-center justify-between">
                  <span>Related News</span>
                  <FaArrowRight className="text-primary-500 text-sm" />
                </h2>
              ...
              <div className="bg-white dark:bg-gray-800 rounded-lg p-6 shadow-lg">
                <h2 className="text-xl font-bold mb-4 text-gray-900 dark:text-white border-b pb-2 flex items-center">
                ...
                {trendingNews.length > 0 ? (
                  <div className="space-y-4">
                      ...
                      >
                        <Link to={`/news/${item._id}`}>
                          <div className="flex items-start space-x-3">
                            {/* Rank Number */}
                            <div className="flex-shrink-0 w-6 h-6 bg-gradient-to-br from-orange-400 to-red-500 rounded-full flex items-center justify-center text-white font-bold text-xs">
                              {index + 1}
                            </div>
                            
                            {/* Content */}
                            <div className="flex-1">
                              <h3 className="text-sm font-medium text-gray-900 dark:text-white group-hover:text-primary-600 dark:group-hover:text-primary-400 line-clamp-2 mb-1">
                                {item.title}
                              </h3>
                    ...
                    {/* View All Link */}
                    <motion.div 
                      className="mt-4 pt-2 border-t border-gray-100 dark:border-gray-700"
                      whileHover={{ x: 5 }}
                    >
                      <Link 
                        to="/trending" 
                        className="text-sm text-primary-600 hover:text-primary-700 dark:text-primary-400 flex items-center justify-between"
                      >
              ...
              {/* Optional: Add an ad in the sidebar */}
              <div className="bg-white dark:bg-gray-800 rounded-lg p-4 shadow-lg">
                <AdManager position="sidebar" page="article" articleId={id} />
              </div>
            </div>
          </motion.aside>
        </div>
